...
(function($) {
  $.fn.filterList = function(ul) {
    ...
    $(this).keyup(function() {
      ...
      var ulTarget = $('#listFilter');
      var filterListTerm = $(this).val();
      if (filterListTerm) {
        ulTarget.find("li:not(:Contains(" + filterListTerm + "))").hide(); //slideUp();
        ulTarget.find("li:Contains(" + filterListTerm + ")").show(); //slideDown();
      } else {
        // Input is blank; show all
        ulTarget.children('li').show(); //slideDown();
      }
      return false;
    });
